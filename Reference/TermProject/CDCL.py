...
class Assignments(dict):
    """
    Use variable as Key to add corresponding Value (Assigment).
...
def cdcl_solve(formula: Formula) -> Optional[Assignments]:
    ...
    Refer to Typical Conflict-Driven Clause Learning Algorithm.
    """
...
# 如果assignments帶入formula後，有找到unit clause，就imply一個隱含的assignment
    ...
    while not finish:
        ...
        for clause in formula:
            ...
            if status == 'unresolved' or status == 'satisfied':
                ...
                # 要出現unit clause的話要長這樣（false, ..., 只有一個未決定的variable, false, ...）
...
def resolve(a: Clause, b: Clause, x: int) -> Clause:
    """
    The resolution operation（resolvent）
    """
...
def conflict_analysis(clause: Clause, assignments: Assignments) -> Tuple[int, Clause]:
    # To achieve Unit Propagation, the depth cannot be 0
    if assignments.dl == 0:
        return (-1, None)
...
def format_assignments(assignments):
    formatted = []
    for var, assignment in sorted(assignments.items()):
        if assignment:
            formatted.append(str(var))
        else:
            formatted.append(f'-{var}')
    return '  '.join(formatted)

if __name__ == '__main__':
    # get inconsistent execution time
    random.seed(5201314)
